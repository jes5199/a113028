...
static void build_oe36(void){
    ...
    for(int j=0;j<npps;j++){
        ...
        if(pps[j].q>pps[j].q1){
            u64 q1=pps[j].q1, q2=pps[j].q2;
            u64 qe3=pps[j].qe[3], qe4=pps[j].qe[4], qe5=pps[j].qe[5], qe6=pps[j].qe[6];
            // Node-level list: e=3 ONLY. The e=4..6 DPs cost ~50-100us/call
            // (gprof: 99.7% of b36 runtime, 73us avg) — far more than they
            // prune at node level. They still run once per subset via
            // subset_filters (unchanged).
            if(qe3>q1 && noe36<MAXOE){ oe36_e[noe36]=3; oe36_q[noe36]=qe3; oe36_tqidx[noe36]=idx; noe36++; }
            (void)qe4; (void)qe5; (void)qe6;
            idx++;
        }
    }
}
