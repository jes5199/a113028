...
static int e2_elig[MAXM], oe36_elig[MAXOE];
    ...
    for(int d=B-1; d>=1; d--){
        ...
        for(int j=0;j<ne2 && ok;j++){
            if(e2_elig[j]){
                ...
                if(w*w >= E2Q[j]-1) continue;   // provably feasible: skip DP
            }
            int t=e2idx[j];
        ...
        for(int i=0;i<noe36 && ok;i++){
            ...
            if(oe36_elig[i]){
                ...
                if(w*w >= q-1) continue;       // provably feasible: skip DP
            }
...
static int engineC(int *nd_result){
    ...
    build_barrett();
    build_layers();
    for(int j=0;j<ne2;j++)  e2_elig[j]  = mod_elig(E2Q[j]);
    for(int i=0;i<noe36;i++) oe36_elig[i]= mod_elig(oe36_q[i]);
