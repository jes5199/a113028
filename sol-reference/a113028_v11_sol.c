...
#define MAXNILPAT 4096
#define MAXNILWORDS ((MAXNILPAT+63)/64)
...
// ---------- exact nilpotent-suffix reservation ----------
// Let Qnil be the product of prime-power factors q|LCM with p|B, and
// T=max ceil(v_p(q)/v_p(B)), so Qnil|B^T.  Every completion must reserve
// T currently-available digits whose SOME ordering in positions 0..T-1
// has value 0 mod Qnil.  Enumerate those unordered masks once per subset.
// During the high-prefix DFS, choosing digit d deletes every reservation
// mask containing d; an empty family is an exact infeasibility certificate.
static u64 nil_pat[MAXNILPAT], nil_uses[MAXB][MAXNILWORDS], nil_active[MAXNILWORDS];
static int nil_npat, nil_words, nil_T, nil_proved_empty;
static u64 nil_Q, nil_w[MAXK];
static long long nil_enum_nodes;
static int nil_enum_abort;

static void nil_add_pattern(u64 mask){
    for(int i=0;i<nil_npat;i++) if(nil_pat[i]==mask) return;
    if(nil_npat>=MAXNILPAT){ nil_enum_abort=1; return; }
    nil_pat[nil_npat++]=mask;
}

static void nil_enum_rec(int pos,u64 used,u64 residue){
    if(nil_enum_abort || ++nil_enum_nodes>5000000) { nil_enum_abort=1; return; }
    if(pos==nil_T){ if(residue%nil_Q==0) nil_add_pattern(used); return; }
    for(int d=1;d<B;d++) if((setmask>>d&1ULL) && !(used>>d&1ULL))
        nil_enum_rec(pos+1,used|(1ULL<<d),(residue+(u64)d*nil_w[pos])%nil_Q);
}

static void build_nil_patterns(void){
    nil_Q=1; nil_T=0; nil_npat=0; nil_words=0; nil_proved_empty=0;
    nil_enum_nodes=0; nil_enum_abort=0;
    for(int j=0;j<npps;j++) if(pps[j].is_nil){
        nil_Q*=pps[j].q; if(pps[j].t>nil_T) nil_T=pps[j].t;
    }
    if(nil_Q<=1 || nil_T<=0 || nil_T>k) return;
    nil_w[0]=1%nil_Q;
    for(int i=1;i<nil_T;i++) nil_w[i]=(nil_w[i-1]*(u64)B)%nil_Q;
    nil_enum_rec(0,0,0);
    if(nil_enum_abort || nil_npat==0){
        // An aborted enumeration is disabled conservatively.  A completed
        // empty enumeration is an exact root-level rejection.
        if(nil_enum_abort){ nil_npat=0; nil_words=0; }
        else nil_proved_empty=1;
        return;
    }
    nil_words=(nil_npat+63)/64;
    memset(nil_uses,0,sizeof(nil_uses)); memset(nil_active,0,sizeof(nil_active));
    for(int i=0;i<nil_npat;i++){
        nil_active[i>>6] |= 1ULL<<(i&63);
        for(int d=1;d<B;d++) if(nil_pat[i]>>d&1ULL)
            nil_uses[d][i>>6] |= 1ULL<<(i&63);
    }
}
...
static int engineC_rec(int m){
    ...
    for(int d=B-1; d>=1; d--){
        ...
        u64 nil_sv[MAXNILWORDS]; int nil_ok=1;
        if(nil_words){
            int any=0;
            for(int z=0;z<nil_words;z++){
                nil_sv[z]=nil_active[z];
                nil_active[z] &= ~nil_uses[d][z];
                any |= nil_active[z]!=0;
            }
            nil_ok=any;
        }
        ...
        c_avail |= 1ULL<<d; if(isD1) c_cntD1++;
        rho_ -= (u128)d*POW_[m-1];
        if(nil_words) for(int z=0;z<nil_words;z++) nil_active[z]=nil_sv[z];
