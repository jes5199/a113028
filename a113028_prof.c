...
// ENGC_PROFILE: sample survivor nodes for the Bridge-Theorem measurement
// protocol (BRIDGE-AND-HARDNESS.md Part I). Logs level m, residual digit
// mask, and prefix residues mod the joint layers; the offline analyzer
// recomputes fibers and surviving-children counts from these.
static int prof_on = -1;
static int engineC_rec(int m){
    if(prof_on<0) prof_on = getenv("ENGC_PROFILE")!=0;
    if(prof_on && m<=gate_ && (((c_nodes)&1023)==0 || m>=gate_-3))
        fprintf(stderr,"P %d %llx %llu %llu\n",
                m,(unsigned long long)c_avail,
                (unsigned long long)ql_res[0],(unsigned long long)ql_res[1]);
    if(++c_nodes > NODE_CAP){
