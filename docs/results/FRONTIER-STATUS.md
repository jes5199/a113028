...
| 54 | 22486771935366632201839109831632289737092127593455046754337233294898735154535035683832000 | **STRONG (zero lex-greater prefixes)** | 522.7s (certset @W22) | **SUPERSEDED the |D|=50 WEAK value** (89 digits vs 87, |D| 51 vs 50, numerically larger). Forced set drops {26,27}. Found at the CHEAPEST rung, W=22, no ladder needed. Maximality structural and needs no census: terminal prefix length 51-23=28, and the value's first 28 digits ARE the descending top-28 of the forced set (…29,28,25,24 — 26 and 27 dropped), i.e. the lex-maximum prefix. **Zero lex-greater prefixes exist** (0 sub-regions, confirmed), the window of 23 = 51-28 covers the entire remainder, so only the equal-prefix branch exists. One computational dependency only. Cheapest possible shape: b63 had 2 free positions (82 branches), b64 had 6 (7 regions), b54 has 0. NOT CERTIFIED (no second engine family). |
...
| 63 | 919638671548642431083440200815272686323739054438990208528434484026726330816976573347252565881592800 | **STRONG (single-method exhaustive, engine-confirmed)** | 136.1s resume-verify (after a 17h shard grind that was later shown unnecessary) | **SUPERSEDED the NO-VALUE row.** Forced set drops {9,18,27,28,36,45,54}, |D|=55, 99 decimal digits. Maximality: 82 lex-relevant terminal prefixes, **81 REFUTED + 1 FOUND** (the incumbent's own branch, maxSurvivor == the incumbent), 0 missing, 0 conflicts — derived independently twice and then reproduced by the engine's own DFS (`nodesVisited=117 found=0 refuted=0 pruned=32 resume-skipped(81/1)`, **zero new terminal executions**). NOT CERTIFIED: one engine family only. See MAXIMALITY-ARGUMENTS.md (same directory). |
...
b57), 5 WEAK lower bounds needing the deep ladder (b54, b59, b61, b62, b64), [SUPERSEDED — see below]
...
> ## ⚠️ The 2026-07-23 climb summary above is HISTORICAL and SUPERSEDED
>
> It is kept as the record of what was known that day. **Every claim in it
> about b54, b59, b61, b62, b63 and b64 has since been overtaken:** all six now
> have values, and all six are **STRONG**. The "deep-ladder class" it calls the
> campaign's open problem was dissolved on 2026-07-25 by §8 of
> `FASTER-PROVISIONAL-MAXIMUM-VALIDATION.md` — the forced maximal-cardinality
> set is larger than each provisional value's set, so any completion supersedes
> by cardinality. Five bases, five completions, five supersessions.
>
> **The live status of every base is the results table above and the README
> ladder, not this snapshot.**
