...
static void build_oe36(void){
    ...
    for(int j=0;j<npps;j++){
        ...
        if(pps[j].q>pps[j].q1){
            ...
            if(qe4>q1 && qe4>q2 && noe36<MAXOE){ oe36_e[noe36]=4; oe36_q[noe36]=qe4; oe36_tqidx[noe36]=idx; noe36++; }
            if(qe5>q1 && noe36<MAXOE){ oe36_e[noe36]=5; oe36_q[noe36]=qe5; oe36_tqidx[noe36]=idx; noe36++; }
            if(qe6>q1 && qe6>q2 && qe6>qe3 && noe36<MAXOE){ oe36_e[noe36]=6; oe36_q[noe36]=qe6; oe36_tqidx[noe36]=idx; noe36++; }
            idx++;
        }
    }
}
...
static int leaf_solve(void){
    int m=P_;
    if(m==0) return (rho_ % LCM)==0;
    u128 v0 = (LCM - (rho_ % LCM)) % LCM;
    u128 span = BP_ - 1 - v0;   // BP_ >= LCM > v0, so BP_-1 >= v0 always
    u128 j = span / LCM;
    int digs[MAXK];
    for(;;){
        u128 v = v0 + j*LCM;
...
static int engineC_rec(int m){
    ...
    for(int d=B-1; d>=1; d--){
        ...
        u64 sv[MAXM];
        for(int j=0;j<ntq;j++){ sv[j]=c_res[j]; c_res[j]=(c_res[j]*(u64)B+(u64)d)%TQ[j]; }
        u128 svrho=rho_; rho_=(rho_+(u128)d*POW_[m-1])%LCM;
